...
`$hunt Check my provider configuration` inspects credentials and prepared dependencies without contacting providers. A request to test live connectivity performs bounded account metadata checks, including Censys organization or account access through its Python SDK. Existing connectivity results are reused; refreshing a report makes new checks. Neither kind of setup is a substitute for an intelligence query.
