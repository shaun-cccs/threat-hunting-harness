...
| Native Codex | The first bounded fixture attempt could delegate but made zero queries: Code Mode host was disabled and role transports were incomplete. Those configuration defects have regression coverage and are fixed. See [verification.md](verification.md) for root's subsequent bounded native check and retained artifacts. A successful fixture call would still not prove OS egress confinement. |
| Native Claude | The bounded native attempt returned `Not logged in`; zero model tokens and zero provider requests were reported. Account login remains an external blocker, distinct from the passing SDK replay; further Claude native verification is deferred by the user. |
